...
struct CourseList: View {
    ...
    var body: some View {
        ZStack {
            Text("\(activeView.height)")
                .bold()
                .offset(y: -350)
                .zIndex(100)
            
            Color.black.opacity(Double(activeView.height / 100))
                .animation(.linear, value: activeView)
            ...
            ScrollView {
                ...
                .animation(.spring(response: 0.5, dampingFraction: 0.6, blendDuration: 0), value: store.courses)
            }
            .statusBar(hidden: active)
            .animation(.linear, value: self.active)
        }
    }
}
...
//        .animation(.spring(response: 0.5, dampingFraction: 0.6, blendDuration: 0), value: activeView)
        .gesture(
            show ?
            DragGesture().onChanged { value in
